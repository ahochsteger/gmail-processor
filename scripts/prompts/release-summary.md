...
- **Standardized Headings**: Use the following headings in this exact order:
  ...
  - `### 📚 Documentation & Examples`: Summarize significant content-related documentation updates or new examples ONLY IF they are explicitly mentioned in the **CHANGELOG** or **PR_SUMMARIES**. Use the **DOCUMENTATION_REFERENCE** strictly as a lookup table to hyperlink topics that were actually modified. **CRITICAL**: Ignore release-related meta-changes, such as auto-generated changelog updates, documentation visibility changes, or build-related documentation patches (e.g., `chore(release)`, `expand changelog visibility`, `update docs patch`). These should be omitted from this section. Do NOT mention links from the reference list unless they relate to an actual functional change in documentation content.
  ...
  - `### 🔮 Looking Ahead`: **MANDATORY**: OMIT this section entirely if **RELEASE_TYPE** is "Maintenance". Otherwise, if **ROADMAP_ITEMS** are provided, synthesize a brief, forward-looking summary of what the project is working on next. Distinguish between focused goals (tagged) and community priorities (trending).
