...
- **Standardized Headings**: Use the following headings in this exact order:
  ...
  - `### 📚 Documentation & Examples`: Summarize significant documentation updates or new examples ONLY IF they are explicitly mentioned in the **CHANGELOG** or **PR_SUMMARIES**. Use the **DOCUMENTATION_REFERENCE** strictly as a lookup table to hyperlink topics that were actually modified. Do NOT mention links from the reference list unless they relate to an actual documented change.
