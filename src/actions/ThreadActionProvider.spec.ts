...
it("should mark a thread as important", () => {
  ...
  const messageActionProvider = new ThreadActionProvider(mockThreadProcessor.processingContext, console, false, mockedGmailThread)
...
it("should not mark a thread as important (dryRun)", () => {
  const mockedGmailThread = mock<GoogleAppsScript.Gmail.GmailThread>()
  const mockThreadProcessor = MockFactory.newThreadProcessorMock(new Config())
  const messageActionProvider = new ThreadActionProvider(mockThreadProcessor.processingContext, console, true, mockedGmailThread)
  messageActionProvider.markImportant()
...
it("should mark a thread as processed by adding a label if processedMode='label'", () => {
  ...
  const messageActionProvider = new ThreadActionProvider(threadProcessor.processingContext, console, false, mockedThread)
