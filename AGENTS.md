...
## OpenSpec Change Workflow Guidelines

- **Archiving Changes**: The `opsx-archive` skill/workflow MUST use the command `openspec archive -y <change_name>` to archive a completed change, instead of directly or manually modifying the directory structure.
- **Explore Analysis Storage**: When starting/creating a new OpenSpec change (using `/opsx-new` or `/opsx-ff`) that follows an exploration session (`/opsx-explore`), the agent MUST automatically store the complete, detailed exploration results in a file named `analysis.md` inside the newly created OpenSpec change directory, alongside other standard change artifacts (e.g., `proposal.md`, `design.md`). This `analysis.md` document MUST contain all in-depth investigation details, empirical evidence, logs, trade-offs, visual diagrams, and option evaluations produced during the exploration, rather than a brief summary.
- **Markdown File Links**: When linking repository files in change artifacts (like proposals, designs, specs, or task lists), the agent MUST use repository-root relative links starting with a leading slash `/` (e.g., `[file.ext](/repo-relative-path/file.ext)`). Do NOT use absolute local `file:///` URLs or directory-relative paths (e.g., `../../src/...`), to ensure links remain portable and unbroken after archiving.
