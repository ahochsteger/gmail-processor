...
class MockThread {
  constructor(private id: string, private date: Date, private subject: string) {}
  getId() {
    getCalls++;
    return this.id;
  }
...
const threads = Array.from({ length: 1000 }, (_, i) => new MockThread(
  `id-${i}`,
  new Date(2020, 0, i % 30),
  `Subject ${i % 100}`
));
...
ThreadProcessor.ordered(
  threads as any,
  { orderBy: ThreadOrderField.LAST_MESSAGE_DATE, orderDirection: OrderDirection.ASC },
